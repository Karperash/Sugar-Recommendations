...
- **График, питание (демо-завтраки), рекомендации, журнал событий; история записей — из раздела «Настройки».**
- **Настройки:** профиль, единицы (мг/дл / ммоль/л), пороги, напоминания; пресет под датасет **Zenodo T1D-UOM** (исследовательский ориентир).
...
### Версия 1.3.1 (versionCode 8)

| Область | Изменения |
|--------|-----------|
| **Рекомендации** | В шапке экрана — иконка больницы; всплывающее окно с **Яндекс Картами** и поиском «больница рядом» (геопозиция определяется сервисом карт). |
| **Манифест** | Разрешение `INTERNET` для загрузки карты в WebView. |
| **Навигация** | Вкладка **«Питание»** вместо истории в нижней панели; **«История»** открывается из **«Настроек»**. |
| **Профиль** | Вес и рост в настройках/анкете; **ИМТ** учитывается вместе с возрастом и полом в логике анализа и подсказках рекомендаций. |
...
| **Рекомендации и UI** | Пресет порогов Zenodo; русские подписи единиц, источников записей, типов событий, трендов, сценариев импорта; `EnumLocalizations` / `ScenarioLocalizations`. |
| **Данные** | Папка `data/zenodo/raw` в `.gitignore`; скрипт `scripts/t1d_uom_glucose_to_diaguide_csv.py`. |
...
*Документ README отражает состояние проекта на момент версии **1.3.1**. Подробная таблица версий — в **`VERSION_HISTORY.txt`***.
